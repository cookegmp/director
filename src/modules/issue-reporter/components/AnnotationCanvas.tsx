...
function AnnotationCanvas({ imageBase64, onComplete, onCancel }: AnnotationCanvasProps) {
  ...
  const handleTextSubmit = () => {
    if (textInput && textInput.text.trim()) {
      setActions([...actions, { type: 'text', x: textInput.x, y: textInput.y, text: textInput.text, color }]);
    }
    setTextInput(null);
  };
          ...
          {([
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                tool === id ? 'bg-primary/20 text-primary' : 'text-muted-foreground hover:text-foreground'
              }`}
              title={label}
            >
