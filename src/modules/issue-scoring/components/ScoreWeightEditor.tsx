    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-xs tabular-nums ${Math.abs(ideaSum - 1) > 0.01 ? 'text-amber-400' : 'text-muted-foreground'}`}>
      ...
      <div className="space-y-3">
        {(Object.entries(IDEA_DIMENSION_LABELS) as [keyof IdeaWeights, string][]).map(([key, label]) => (
          <WeightSlider
            key={key}
            label={label}
