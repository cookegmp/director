...
async function scoreWithAI(
  ...
  return [
    ...
    buildDimensionScore('alignment', parsed.alignment.score, weights.alignment, parsed.alignment.explanation),
    buildDimensionScore('complexity', parsed.complexity.score, weights.complexity, parsed.complexity.explanation),
...
function scoreWithRules(
  ...
  if (descLower.includes('team') || descLower.includes('everyone') || descLower.includes('department')) demandScore = Math.max(demandScore, 65);
  if (descLower.includes('personal') || descLower.includes('just me') || descLower.includes('i would')) demandScore = Math.min(demandScore, 30);
  ...
  if (descLower.includes('minor') || descLower.includes('simple') || descLower.includes('toggle') || descLower.includes('config')) {
    complexityScore = 85;
  } else if (descLower.includes('extend') || descLower.includes('add option') || descLower.includes('new column')) {
    complexityScore = 70;
  } else if (descLower.includes('new component') || descLower.includes('new page')) {
    complexityScore = 50;
  } else if (descLower.includes('database') || descLower.includes('integration') || descLower.includes('new api')) {
    complexityScore = 30;
  } else if (descLower.includes('architecture') || descLower.includes('redesign') || descLower.includes('migration')) {
    complexityScore = 15;
  }

  // Impact: workflow improvement signals
  let impactScore = 50;
  if (descLower.includes('daily') || descLower.includes('every day') || descLower.includes('constantly')) impactScore = 80;
  else if (descLower.includes('new capability') || descLower.includes('not possible') || descLower.includes('cannot currently')) impactScore = 70;
  else if (descLower.includes('occasional') || descLower.includes('sometimes')) impactScore = 50;
  else if (descLower.includes('cosmetic') || descLower.includes('nice to have') || descLower.includes('convenience')) impactScore = 30;

  return [
    buildDimensionScore('demand', demandScore, weights.demand, `${similarFeatures.length} similar request(s) found`),
    buildDimensionScore('alignment', alignmentScore, weights.alignment, context.charter ? 'Evaluated against charter objectives' : 'No charter context available'),
    buildDimensionScore('complexity', complexityScore, weights.complexity, 'Estimated from description keywords (inverse: higher = simpler)'),
    buildDimensionScore('impact', impactScore, weights.impact, 'Estimated from workflow impact signals'),
  ];
}
