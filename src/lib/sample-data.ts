...
export const sampleIdeas: Idea[] = [
  ...
  },
  {
    ...
    intakeAnswers: {
      ...
      'current-state': 'Documents are spread across SharePoint Online, network file shares (SMB), and a folder-based QMS system. Each has its own search mechanism. No cross-system search capability. File naming conventions are inconsistent.',
      'desired-outcome': 'A single search interface that queries all document repositories. Results ranked by relevance. Preview documents without opening. Filter by document type, project, date range. Bookmark frequently accessed documents.',
      constraints: 'Must integrate with Microsoft 365 / SharePoint API. Read-only access to network shares. Must respect existing access permissions. Cannot modify the underlying document storage systems.',
      urgency: 'Causing daily friction',
    },
  ...
  },
  {
    ...
    intakeAnswers: {
      ...
      'current-state': 'Engineers manually read PDF/DOCX specifications, highlighting key requirements and transcribing them into project requirement documents. Some use ctrl+F to search but there\'s no structured extraction.',
      'desired-outcome': 'Upload a customer spec document. AI extracts key requirements: materials, tolerances, surface finishes, testing requirements, certifications needed, delivery requirements. Structured output that can be reviewed and exported to project planning.',
      constraints: 'Must handle PDF and DOCX formats. Customer documents are confidential and must stay on-premises. Extraction accuracy must be verifiable — engineers review AI output. Must handle GD&T notation and manufacturing-specific terminology.',
      urgency: 'Would improve efficiency',
    },
...
export const sampleCharters: Charter[] = [
  ...
  },
  {
    ...
    content: {
      ...
      technicalApproach: 'The application will use React with TypeScript for the frontend with a Node.js GraphQL backend. Microsoft Graph API will handle SharePoint document access. Network share indexing will use a background service that periodically scans configured paths. Document metadata will be stored in MS SQL Server with full-text search capabilities. For enhanced search, document content will be extracted and indexed for keyword matching.',
      acceptanceCriteria: [
      ...
      estimatedTimeline: '6 weeks for MVP with SharePoint integration, 10 weeks for full multi-source support',
      executionPlan: [
        {
...
export const sampleAgentSessions: AgentSession[] = [
  {
    ...
    stoppedAt: null,
    translatedEntries: [
      { id: 'te-001', timestamp: hoursAgo(20), summary: 'Installing required components', type: 'progress', phase: 'Document Processing Foundation', rawLineIndex: 0 },
      { id: 'te-002', timestamp: hoursAgo(20), summary: 'Setting up project structure', type: 'progress', phase: 'Document Processing Foundation', rawLineIndex: 1 },
      { id: 'te-003', timestamp: hoursAgo(19.5), summary: 'Configuring build system', type: 'progress', phase: 'Document Processing Foundation', rawLineIndex: 2 },
      { id: 'te-004', timestamp: hoursAgo(19), summary: 'Setting up TypeScript configuration', type: 'progress', phase: 'Document Processing Foundation', rawLineIndex: 3 },
      { id: 'te-005', timestamp: hoursAgo(18.5), summary: 'Built the upload API endpoint', type: 'milestone', phase: 'Document Processing Foundation', rawLineIndex: 4 },
      { id: 'te-006', timestamp: hoursAgo(18), summary: 'Created PDF text extraction pipeline', type: 'milestone', phase: 'Document Processing Foundation', rawLineIndex: 5 },
      { id: 'te-007', timestamp: hoursAgo(17.5), summary: 'Created DOCX text extraction pipeline', type: 'milestone', phase: 'Document Processing Foundation', rawLineIndex: 6 },
      { id: 'te-008', timestamp: hoursAgo(17), summary: 'Built document chunking for LLM context', type: 'progress', phase: 'Document Processing Foundation', rawLineIndex: 7 },
      { id: 'te-009', timestamp: hoursAgo(16.5), summary: 'Configuring OpenRouter integration', type: 'progress', phase: 'Document Processing Foundation', rawLineIndex: 8 },
      { id: 'te-010', timestamp: hoursAgo(16), summary: 'Encountered an issue -- working through it', type: 'error', phase: 'AI Extraction Engine', rawLineIndex: 9 },
      { id: 'te-011', timestamp: hoursAgo(15.5), summary: 'Fixing: missing type definition for extraction schema', type: 'recovery', phase: 'AI Extraction Engine', rawLineIndex: 10 },
      { id: 'te-012', timestamp: hoursAgo(15), summary: 'Issue resolved -- moving forward', type: 'recovery', phase: 'AI Extraction Engine', rawLineIndex: 11 },
      { id: 'te-013', timestamp: hoursAgo(14.5), summary: 'Built extraction prompt with manufacturing domain knowledge', type: 'milestone', phase: 'AI Extraction Engine', rawLineIndex: 12 },
      { id: 'te-014', timestamp: hoursAgo(14), summary: 'Created structured output parser', type: 'progress', phase: 'AI Extraction Engine', rawLineIndex: 13 },
      { id: 'te-015', timestamp: hoursAgo(13.5), summary: 'All 8 quality checks passed', type: 'milestone', phase: 'AI Extraction Engine', rawLineIndex: 14 },
    ],
    rawOutput: [
      { index: 0, timestamp: hoursAgo(20), content: 'npm install -- installing 18 packages' },
      { index: 1, timestamp: hoursAgo(20), content: 'mkdir -p src/components src/pages src/hooks src/lib src/types' },
      ...
      { index: 4, timestamp: hoursAgo(18.5), content: 'Created src/pages/UploadPage.tsx -- document upload API endpoint' },
      { index: 5, timestamp: hoursAgo(18), content: 'Created src/lib/pdf-extractor.ts -- PDF text extraction' },
      { index: 6, timestamp: hoursAgo(17.5), content: 'Created src/lib/docx-extractor.ts -- DOCX text extraction' },
      { index: 7, timestamp: hoursAgo(17), content: 'Created src/lib/chunker.ts -- document chunking strategy' },
      { index: 8, timestamp: hoursAgo(16.5), content: 'Writing src/lib/openrouter.ts -- OpenRouter integration' },
      { index: 9, timestamp: hoursAgo(16), content: 'Error: TypeScript compilation failed -- cannot find ExtractionSchema type' },
      { index: 10, timestamp: hoursAgo(15.5), content: 'fixing: adding ExtractionSchema type definition to types/index.ts' },
      { index: 11, timestamp: hoursAgo(15), content: 'resolved -- TypeScript compilation successful' },
      { index: 12, timestamp: hoursAgo(14.5), content: 'Created src/lib/extraction-prompt.ts -- manufacturing domain extraction' },
      { index: 13, timestamp: hoursAgo(14), content: 'Created src/lib/output-parser.ts -- structured requirement parser' },
      { index: 14, timestamp: hoursAgo(13.5), content: '8/8 tests passed' },
    ],
    errors: [
      { timestamp: hoursAgo(16), message: 'TypeScript compilation error: cannot find ExtractionSchema type', recoverable: true },
    ],
  },
];
...
export const sampleIssues: Issue[] = [
  ...
  },
  {
    ...
    description: 'When an operator completes a job operation, the dashboard shows "Unknown" instead of the operator name. The data is correctly stored in the database but the resolver is not joining the Users table.',
    severity: 'medium',
    status: 'resolved',
    projectId: 'charter-001',
    createdAt: daysAgo(8),
  ...
  },
  {
    ...
    description: 'Managers want to see machine utilization percentages based on job operation time tracking data. This would show how effectively each machine/workcenter is being used.',
    severity: 'medium',
    status: 'open',
    projectId: 'charter-001',
    createdAt: daysAgo(3),
...
export const sampleAISettings: AISettings = {
  ...
  issueScoringModel: 'anthropic/claude-haiku',
  issueScoringEnabled: true,
