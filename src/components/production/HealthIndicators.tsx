...
function HealthIndicators({ projectId }: HealthIndicatorsProps) {
  ...
  const openBugs = projectIssues.filter((i) => i.type === 'bug' && (i.status === 'open' || i.status === 'in-progress'));
  const criticalBugs = openBugs.filter((i) => i.severity === 'critical');
  const featureRequests = projectIssues.filter((i) => i.type === 'feature-request' && i.status !== 'closed');

  // Calculate avg resolution time from resolved/closed bugs
  const resolvedBugs = projectIssues.filter((i) => i.type === 'bug' && (i.status === 'resolved' || i.status === 'closed'));
  ...
  const trend = resolvedBugs.length > openBugs.length ? 'decreasing' : resolvedBugs.length === openBugs.length ? 'stable' : 'increasing';

  const bugCountColor = criticalBugs.length > 0 || openBugs.length >= 6
    ? 'text-red-400'
    : openBugs.length >= 3
      ? 'text-amber-400'
      : 'text-green-400';

  const resolutionColor = avgResolutionDays > 7
    ? 'text-red-400'
    : avgResolutionDays > 3
      ? 'text-amber-400'
      : 'text-green-400';

  const trendColor = trend === 'increasing' ? 'text-red-400' : trend === 'stable' ? 'text-amber-400' : 'text-green-400';
  const TrendIcon = trend === 'increasing' ? TrendingUp : trend === 'decreasing' ? TrendingDown : Minus;

  // Avg Bug Score
  const bugIssueIds = projectIssues.filter((i) => i.type === 'bug').map((i) => i.id);
  const bugScores = allScores.filter((s) => bugIssueIds.includes(s.issue_id) && s.score_type === 'bug');
  const avgBugScore = bugScores.length > 0
    ? Math.round(bugScores.reduce((sum, s) => sum + s.composite_score, 0) / bugScores.length)
    : null;
  const avgScoreColor = avgBugScore === null
    ...
    <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <div className="bg-card/50 backdrop-blur-sm rounded-xl border border-border p-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs text-muted-foreground uppercase tracking-wide">Avg Resolution</span>
        </div>
      ...
      <div className="bg-card/50 backdrop-blur-sm rounded-xl border border-border p-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs text-muted-foreground uppercase tracking-wide">Feature Requests</span>
        </div>
      ...
      <div className="bg-card/50 backdrop-blur-sm rounded-xl border border-border p-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="text-xs text-muted-foreground uppercase tracking-wide">Avg Bug Score</span>
        </div>
