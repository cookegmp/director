...
import type { AgentSession, AgentSessionStatus, TranslatedEntry, RawOutputLine, AgentError } from '@/types';
    ...
    (set, get) => ({
      ...
      appendTranslatedEntry: (id, entry) =>
        set((state) => ({
          sessions: state.sessions.map((s) =>
            s.id === id
              ? { ...s, translatedEntries: [...s.translatedEntries, entry] }
              : s
          ),
        })),
      ...
      getSessionByIdeaId: (ideaId) =>
        get().sessions.find((s) => s.ideaId === ideaId),
    }),
